...
def __remove_self_assign(i, instruction, block):
    ...
    if (instruction.opcode == "AND" and instruction.reads[1] == WORD_MASK
...
def __rewrite_shift(i, instruction, block):
    ...
    if opcode == "DIV":
        ...
        new_instruction = Instruction("SR", [instruction.reads[0], exp],
        ...
        if not (((num & (num - 1)) == 0) and num > 256):
            return
        exp = int(math.log(num, 2))
        new_instruction = Instruction("SL", [instruction.reads[1], exp],
                                      instruction.writes, address)
        block.set_instruction(i, new_instruction)
...
def __remove_address_mask(i, instruction_0, instruction_1, block):
    if (instruction_0.opcode not in {"CALLER", "ADDRESS"}
...
def __remove_doube_iszero(i, instruction_0, instruction_1, block):
    if (instruction_0.opcode != "ISZERO" or instruction_1.opcode != "ISZERO"
...
def __rewrite_negate_ops(i, instruction_0, instruction_1, block):
    ...
    if (instruction_0.opcode not in negate_ops
            or instruction_1.opcode != "ISZERO"
            or instruction_1.address != instruction_0.address + 1):
        return
    block.set_nop_instruction(i - 1)
    opcode = negate_ops[opcode]
    new_instruction = Instruction(opcode, instruction_0.reads,
...
class Optimizer(Lifter):
    ...
    def get_liveness_states(self, func):
        ...
        if isinstance(
                func,
                InternalFunction):  # must keep all return registers alive
