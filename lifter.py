...
class Lifter(GraphBuilder):
    ...
    def __create_internal_function(self, callee_pair, caller_pairs):
        ...
        for caller_pair in caller_pairs:
            ...
            interpreter = BasicInterpreter(self.graph.get_blocks(),
                                           self.resolver)
            ...
            in_func = InternalFunction(signature, sub_graph, sub_tracker,
                                       callee_pair, operations)
        ...
        caller_pairs, func = max(possible_funcs.values(),
    ...
    def __reduce_functions(self):
        for func in self.get_all_functions():
            ...
            interpreter = DuplicateInterpreter(
                func.graph.get_blocks(),
                self.resolver.get_natural_successors())
    ...
    def __lift_bytecode_block(self, block, stack_size):
        ...
        for bytecode in block:
            instructions, stack_size = self.__lift_bytecode(
                bytecode, stack_size)
    ...
    def __lift_bytecode(bytecode, stack_size):
        ...
        if opcode in swap_ops:
            ...
            instructions = [
                MoveInstruction("MOVE", [constant], writes, address)
            ]
            ...
            instructions = [
                CallLoadInstruction(opcode, reads, writes, address)
            ]
